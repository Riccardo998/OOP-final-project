...
public class StudioDbAdapter extends RecyclerView.Adapter<StudioDbAdapter.StudioDbViewHolder> {
    ...
    public void setOnItemClickListener(StudioDbAdapter.OnItemClickListener listener) {
        mListener = listener;
    }
    ...
    @Override
    public StudioDbAdapter.StudioDbViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View v = LayoutInflater.from(parent.getContext()).inflate(R.layout.example_item, parent, false);
        StudioDbAdapter.StudioDbViewHolder
                evh = new StudioDbAdapter.StudioDbViewHolder(v, mListener);
        return evh;
    }

    @Override
    public void onBindViewHolder(StudioDbAdapter.StudioDbViewHolder holder, int position) {

        if (!mCursor.moveToPosition(position)) {
            return;
        }
        //PRENDO ATTIVITA SQLITE E LA MOSTRO A DISPLAY
        String name = mCursor.getString(mCursor.getColumnIndex(StudioEntry.COLUMN_ATTIVITA));
